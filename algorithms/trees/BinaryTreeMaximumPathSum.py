...
# how to decide the maxmum length of path rooted at this node?
# it must be Max(node.left.maxval,0) + Max(node.right.maxval,0) + node.val
# where compair to 0 means if the max val of child node is negative, then abandon this child node.
#
# Idea:
...
# Any node should return ether current maxmum value or 0 if current max is negative.
